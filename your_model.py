...
class YourModel(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        logits = Conv2D('conv0', image, 32, (5,5), padding='same', stride=(1,1), nl=tf.nn.relu)
        #
        ...
        logits = MaxPooling('pool0', logits, (4,4), stride=(2,2), padding='same')

        logits = Conv2D('conv1', logits, 64, (5, 5), padding='same', stride=(1,1), nl=tf.nn.relu)

        logits = MaxPooling('pool1', logits, (4,4), stride=(2,2), padding='same')

        logits = Dropout('dropout', logits, 0.5)
        #
